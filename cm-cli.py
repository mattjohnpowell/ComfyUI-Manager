...
def restore_snapshot(
    ...
    try:
        ...
        with open(snapshot_path, 'r', encoding="UTF-8") as snapshot_file:
            ...
            if cnr_info is not None:
                ...
                for k, v in cnr_info.items():
                    ps = asyncio.run(unified_manager.install_by_id(k, version_spec=v, instant_execution=True, return_postinstall=True))
                    if ps is not None:
        ...
        # reload
        asyncio.run(unified_manager.reload(cache_mode=channel_ctx.mode == 'cache'))
